...
void * app_realloc(void *ptr, long bytes)
{
	app_free(ptr);
	return malloc(bytes);
}
...
FILE * app_open_file(const char *name, const char *mode)
{
	return fopen(name, mode);
}

void app_close_file(FILE *fp)
{
	fclose(fp);
}

Colour rgb(byte r, byte g, byte b)
{
	Colour tmp;
	tmp.red = r;
	tmp.green = g;
...
void print_gif_palette(FILE *file, GifPalette *cmap)
{
	...
	for (i=0; i<cmap->length; i++) {
		...
		//fprintf(file, "%02X\n", cmap->colours[i].blue);
		fputc(cmap->colours[i].red / 4, file);
		fputc(cmap->colours[i].green / 4, file);
		fputc(cmap->colours[i].blue / 4, file);
	}
}
#endif
...
void write_gif_picture_data(FILE *file, GifPicture *pic)
{
	GifEncoder *encoder;
	long w, h;
	int interlace_start[] = {0, 4, 2, 1};
...
#ifdef GIF_DEBUG
void print_gif_picture_data(FILE *file, GifPicture *pic)
{
	int pixval, row, col;
